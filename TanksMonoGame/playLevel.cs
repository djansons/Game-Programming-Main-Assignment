...
namespace Tanks
{
    class playLevel : RC_GameStateParent
    {
        ...
        Texture2D texMap1 = null;
        Texture2D texTrain = null;
        ...
        float ET1StartPosX = 100;
        float ET1StartPosY = 100;
        float ET2StartPosX = 300;
        float ET2StartPosY = 100;
        float mouseX, mouseY;
        float tankSpeed = 1; // needs to be 1 for collisions to work with train - Do not touch
        float lineRotateSpeed = 1;
        float degrees = 0;      
        ...
        public override void Initialize()
        {
            ...
            Map1 = new Rectangle(0, 0, graphicsDevice.Viewport.Width, graphicsDevice.Viewport.Height);
            playArea = new Rectangle(playAreaHorizontalOffset, 33, graphicsDevice.Viewport.Width -(playAreaHorizontalOffset*2), graphicsDevice.Viewport.Height-165);
            tempPos = new Vector2(0, playArea.Height);
            train = new Rectangle(0, 300, 500, 100);
        ...
        public override void LoadContent()
        {
            ...
            texMap1 = Content.Load<Texture2D>("map1");
            texTrain = Content.Load<Texture2D>("train");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (keyState.IsKeyDown(Keys.W))
            {
                ...
                rectCollision = didRectangleCollide(train, tankBody);
            ...
            if (playerHit == false) // So we only get little explosion if enemy did not hit player tank. 
            {
                ...
                for (int i = 0; i < smallMissleList.count(); i++)
                {
                    ...
                    calcSurfaceCollission(temp, train);
                    ...
                    if (temp.hitPoints >= temp.maxHitPoints && (tankBody.collision(temp) == false) && temp.active == true)
                    {
                        temp.active = false;
                        CreateSmallExplosion(temp.getPosX(), temp.getPosY());
                    }

                    for (int count1 = 0; count1 < playerMissleList.countActive(); count1++)
                    {
                        Sprite3 playerTemp = playerMissleList.getSprite(count1);
                ...
                for (int count = 0; count < playerMissleList.count(); count++)
                {
                    ...
                    calcSurfaceCollission(playerTemp, train);

                    //if (playerTemp.hitPoints == playerTemp.maxHitPoints)
                        if (playerTemp.hitPoints == playerTemp.maxHitPoints && (tankBody.collision(playerTemp) == false) && playerTemp.active == true)
                        {
                        playerTemp.active = false;
                        CreateSmallExplosion(playerTemp.getPosX(), playerTemp.getPosY());
                    }
            ...
            calcLine1Obstruction(train, ET1);
            calcLine1Obstruction(train, ET2);
            ...
            calcLine2Obstruction(train, ET1);
            calcLine2Obstruction(train, ET2);
            calcLine3Obstruction(train, ET1);
            calcLine3Obstruction(train, ET2);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Draw(texMap1, Map1, Color.White);
            spriteBatch.Draw(texTrain, train, Color.White);

            if (Game1.showBB)
            {
                LineBatch.drawLineRectangle(spriteBatch, train, Color.Pink);
        ...
        private void CreateMissle(EnemyTank ET, float misslePosX, float misslePosY, bool enemyMissle)
        {
            if (enemyMissle)
            {
                ...
                bigMissle.setHSoffset(new Vector2(20, 0));
                bigMissle.setWidthHeight(17, 35);
                bigMissle.setBBToTexture();
                bigMissle.maxHitPoints = 2;
        ...
        private int didRectangleCollide(Rectangle wall, Sprite3 sprite)
        {
            ...
            enemyLineIntersect = Util.intersect2D(ref x, ref y, sprite.getBoundingBoxAA().Right, sprite.getBoundingBoxAA().Y, sprite.getBoundingBoxAA().Bottom, sprite.getBoundingBoxAA().Right, wall.X, wall.Y, wall.X, wall.Y + wall.Height);//2 LH side of rectangle collision
            if (enemyLineIntersect == 1)
            {
                stuckSide = 2;
            }
        ...
        private void calcLine1Obstruction(Rectangle wall, EnemyTank ET)
        {
            ...
            if ((uint)(ET.EnemyLineStart.Y - wall.Bottom) > (uint)(ET.EnemyLineStart.Y - wall.Top)) // Calcs if top of rectangle or bottom is closer to line start. 
            {
        ...
        public void calcSurfaceCollission(Sprite3 rocket, Rectangle wall)
        {
            ...
            double x = 0;
            double y = 0;
            float temp3 = Convert.ToInt32(rocket.getPosX());
            

            enemyLineIntersect = Util.intersect2D(ref x, ref y, temp3, rocket.getPosY(), temp3+3, rocket.getPosY(), wall.X + wall.Width, wall.Y, wall.X + wall.Width, wall.Y + wall.Height);
            //enemyLineIntersect = Util.intersect2D(ref x, ref y, rocket.getPosX(), rocket.getPosY(), rocket.getPosX() + 3, rocket.getPosY(), wall.X + wall.Width+1, wall.Y, wall.X + wall.Width+1, wall.Y + wall.Height);
            //enemyLineIntersect = Util.intersect2D(ref x, ref y, rocket.getPosX(), rocket.getPosY(), rocket.getPosX() + 3, rocket.getPosY(), wall.X + wall.Width -1, wall.Y, wall.X + wall.Width -1, wall.Y + wall.Height);
            //enemyLineIntersect = Util.intersect2D(ref x, ref y, rocket.getPosX(), rocket.getPosY(), rocket.getPosX() + 3, rocket.getPosY(), wall.X + wall.Width - 2, wall.Y, wall.X + wall.Width - 2, wall.Y + wall.Height);
           // enemyLineIntersect = Util.intersect2D(ref x, ref y, rocket.getPosX(), rocket.getPosY(), rocket.getPosX() + 3, rocket.getPosY(), wall.X + wall.Width - 3, wall.Y, wall.X + wall.Width - 3, wall.Y + wall.Height);//3 RH side of rectangle collision// second time to ensure colision with train. 
            if (enemyLineIntersect == 0)
            {
        ...
        private void moveEnemyLine(EnemyTank ET) // This makes the enemy line of sight move around the map. 
        {
            ...
            {
                tempPos.Y = playArea.Height + playAreaVerticalOffset;
                tempPos.X -= lineRotateSpeed;
